...
export default function NotificationsPage() {
  ...
  const { data, isLoading: loading } = useCustomerNotificationsQuery(page, unreadOnly);
  const { markNotificationRead, markAllRead } = useCustomerMutations();
  const notifications: CustomerNotificationItem[] = data?.notifications ?? [];
  const totalPages = data?.totalPages ?? 1;

  const handleClick = async (n: CustomerNotificationItem) => {
    if (!n.isRead) {
      try {
  ...
  const markingAll = markAllRead.isPending;
