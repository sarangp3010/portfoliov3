# In Progress

This document tracks tasks that are currently being worked on. Move a task here from `upcoming.md` when you start it, and move it to `completed.md` when it is done.

---

## How to use this file

1. Pick a task from `upcoming.md`
2. Copy it here and fill in the **Started** date and **Branch** name
3. Use the **Notes** section to track decisions, blockers, and sub-steps as you work
4. When done, move the full entry to `completed.md` with the final commit hash and summary

---

<!-- Paste active tasks below this line -->
